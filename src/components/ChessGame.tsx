...
const ChessGame = () => {
      ...
      const fenRows = rows.map((row: string) => {
        ...
        for (let i = 0; i < row.length; i++) {
          if (row[i] === '.') {
            ...
            fenRow += row[i] === row[i].toUpperCase() ? row[i].toLowerCase() : row[i].toUpperCase();
          }
        }
  ...
  const getSquareStyles = () => {
    ...
    if (gameState.lastMove) {
      squares[gameState.lastMove.from] = { backgroundColor: 'rgba(255, 255, 0, 0.4)' };
      squares[gameState.lastMove.to] = { backgroundColor: 'rgba(255, 255, 0, 0.4)' };
    }
  ...
  return (
    <div className="chess-game" style={{ textAlign: 'center', padding: '20px' }}>
